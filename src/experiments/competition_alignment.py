...
ALIGNMENT_SPECS = [
    ...
    AlignmentSpec(
        ...
        implemented_mechanism=(
            ...
            "ablation isolates which action protects which mission metric."
        ),
        ...
        row_checks=[RowCountCheck("outputs/batch/tsra_action_ablation_summary.csv", 5)],
    ),
    AlignmentSpec(
        alignment_id="A04",
        ...
        next_gate="Agent changes must leave DecisionTrace evidence for action, reason, tool calls, and feedback.",
        row_checks=[RowCountCheck("outputs/report_tables/agent_decision_trace_summary.csv", 200)],
    ),
    AlignmentSpec(
        alignment_id="A05",
        scoring_area="Attack-defense cooperation",
        competition_goal="Make the red and blue agents observable on one shared event timeline.",
        implemented_mechanism=(
            "Battle timeline and incident summary merge AURA events, TSRA-R events, trace reasons, "
            "and metric movement for E5 and E7."
        ),
        agent_or_component="Battle timeline / Incident summary",
        evidence_files=[
        ...
        next_gate="New experiments must preserve attack events, defense events, trace reasons, and metric snapshots.",
        content_checks=[
